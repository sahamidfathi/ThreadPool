...
class ThreadPool {
	...
	ThreadSafeQueue<FunctionWrapper> poolQueue;
	typedef std::queue<FunctionWrapper> localQueueType;
	static thread_local std::unique_ptr<localQueueType> localQueue;
	std::vector<std::thread> threads;
	JoinThreads joiner;

	void workerThread() {
		localQueue.reset(new localQueueType);
		while (!done) {
			runPendingTask();
		}
	}
...
public:
	...
	void runPendingTask() {
        ...
        if(localQueue && !localQueue->empty()) {
			task = std::move(localQueue->front());
			localQueue->pop();
			task();
		} else if(poolQueue.tryPop(task)) {
            task();
        } else {
...
//thread_local std::unique_ptr<ThreadPool::localQueueType> ThreadPool::localQueue = nullptr;
